 ...
 * @param[in] domain Domain indentifier to be set.
 */
 ...
 */
NRF_STATIC_INLINE nrf_domain_t nrf_irqmap_sink_get(NRF_IRQMAP_Type const * p_reg,
                                                   uint32_t                index);
...
{
    NRFX_ASSERT(index < NRF_IRQMAP_IRQ_COUNT);
    return (nrf_domain_t)((p_reg->IRQ[index].SINK & IRQMAP_IRQ_SINK_DOMAIN_Msk)
