...
#if defined(GRCCONF_SYSTEMOFF_SYSTEMOFFIGNORESECDOM_Msk) || defined(__NRFX_DOXYGEN__)
/** @brief Symbol indicating whether SYSTEMOFFIGNORESECDOM bit is present. */
#define NRF_GRCCONF_HAS_SYSTEMOFFIGNORESECDOM 1
#else
#define NRF_GRCCONF_HAS_SYSTEMOFFIGNORESECDOM 0
#endif

#if defined(GRCCONF_SYSTEMOFF_FORCEGDOFF_Msk) || defined(__NRFX_DOXYGEN__)
/** @brief Symbol indicating whether SYSTEMOFF_FORCEGDOFF bit is present. */
#define NRF_GRCCONF_HAS_FORCEGDOFF 1
#else
#define NRF_GRCCONF_HAS_FORCEGDOFF 0
#endif

#if defined(GRCCONF_EVENTS_SYSTEMOFFLOCALDOMAINOFF_EVENTS_SYSTEMOFFLOCALDOMAINOFF_Msk) || \
    defined(__NRFX_DOXYGEN__)
/** @brief Symbol indicating whether SYSTEMOFFLOCALDOMAINOFF bit is present. */
#define NRF_GRCCONF_HAS_SYSTEMOFFLOCALDOMAINOFF 1
#else
#define NRF_GRCCONF_HAS_SYSTEMOFFLOCALDOMAINOFF 0
#endif

#if defined(GRCCONF_PDREQSTATUS_MaxCount) || defined(__NRFX_DOXYGEN__)
/** @brief Symbol indicating whether PDREQSTATUS is a single register or an array. */
#define NRF_GRCCONF_HAS_PDREQSTATUS_ARRAY 1
#else
#define NRF_GRCCONF_HAS_PDREQSTATUS_ARRAY 0
#endif
/**
...
#if NRF_GRCCONF_HAS_SYSTEMOFFLOCALDOMAINOFF
    NRF_GRCCONF_EVENT_SYSTEMOFF_SYSTEMOFFLOCALDOMAINOFF = offsetof(NRF_GRCCONF_Type, EVENTS_SYSTEMOFFLOCALDOMAINOFF), /**< All local power domains are powered off. */
#endif
} nrf_grcconf_event_t;
...
/**
 * @brief Function for setting bits in the SYSTEMOFF register.
 *
 * @param[in] p_reg Pointer to the structure of registers of the peripheral.
 * @param[in] mask  Mask of SYSTEMOFF bits to be set.
 */
NRF_STATIC_INLINE void nrf_grcconf_system_off_set(NRF_GRCCONF_Type * p_reg, uint32_t mask);

/**
 * @brief Function for clearing bits in the SYSTEMOFF register.
 *
 * @param[in] p_reg Pointer to the structure of registers of the peripheral.
 * @param[in] mask  Mask of SYSTEMOFF bits to be cleared.
 */
NRF_STATIC_INLINE void nrf_grcconf_system_off_clear(NRF_GRCCONF_Type * p_reg, uint32_t mask);
 ...
 * @param[in] p_reg Pointer to the structure of registers of the peripheral.
 */
NRF_STATIC_INLINE void nrf_grcconf_system_off_enter_system_off(NRF_GRCCONF_Type * p_reg);
...
#if NRF_GRCCONF_HAS_SYSTEMOFFIGNORESECDOM
/**
 * @brief Function for setting whether power state of secure domain should be ignored.
 *
 * @details Power state of secure domain is ignored for SYSTEMOFFLOCALDOMAINOFF and
 *          secure domain is allowed to both run during and wake up again after SYSTEMOFFLOCALDOMAINOFF.
 *
 * @param[in] p_reg Pointer to the structure of registers of the peripheral.
 */
NRF_STATIC_INLINE void nrf_grcconf_system_off_secdom_ignore_set(NRF_GRCCONF_Type * p_reg);
#endif

/**
 * @brief Function for retrieving power domain request status.
 *
 * @param[in] p_reg     Pointer to the structure of registers of the peripheral.
 * @param[in] group_idx Index of PDREQSTATUS array.
 * @param[in] mask      Mask of power domain status to be checked.
 *
 * @return Bitmask with information which power domain request status is active.
 */
NRF_STATIC_INLINE uint32_t nrf_grcconf_pdreqstatus_get(NRF_GRCCONF_Type const * p_reg,
                                                       uint8_t                  group_idx,
                                                       uint32_t                 mask);
...
NRF_STATIC_INLINE void nrf_grcconf_force_system_off(NRF_GRCCONF_Type * p_reg, bool now)
{
    if (now)
    {
        p_reg->SYSTEMOFF = (GRCCONF_SYSTEMOFF_FORCESYSTEMOFFNOW_Force
    ...
    {
        p_reg->SYSTEMOFF =
#if NRF_GRCCONF_HAS_FORCEGDOFF
            (GRCCONF_SYSTEMOFF_FORCEGDOFF_Force
                    << GRCCONF_SYSTEMOFF_FORCEGDOFF_Pos) & GRCCONF_SYSTEMOFF_FORCEGDOFF_Msk;
#else
...
{
    p_reg->SYSTEMOFF =
        (GRCCONF_SYSTEMOFF_SYSTEMOFFIGNORESECDOM_Enable << GRCCONF_SYSTEMOFF_SYSTEMOFFIGNORESECDOM_Pos)
         & GRCCONF_SYSTEMOFF_SYSTEMOFFIGNORESECDOM_Msk;
}
#endif
...
NRF_STATIC_INLINE uint32_t nrf_grcconf_pdreqstatus_get(NRF_GRCCONF_Type const * p_reg,
                                                       ...
                                                       uint32_t                 mask)
{
#if NRF_GRCCONF_HAS_PDREQSTATUS_ARRAY
    NRFX_ASSERT(group_idx < GRCCONF_PDREQSTATUS_MaxCount);
    return p_reg->PDREQSTATUS[group_idx] & mask;
#else
    (void)group_idx;
    return p_reg->PDREQSTATUS & mask;
#endif
}
