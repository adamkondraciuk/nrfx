 ...
 *
 * Here is a sample of using the NRFX_GPIOTE_INST_HANDLER_GET macro to directly map
 * an interrupt handler in a Zephyr application:
 *
 * IRQ_DIRECT_CONNECT(NRFX_IRQ_NUMBER_GET(NRF_GPIOTE_INST_GET(\<instance_index\>)), \<priority\>,
 *                    NRFX_GPIOTE_INST_HANDLER_GET(\<instance_index\>), 0);
 */
