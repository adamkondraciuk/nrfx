    ...
    while (0 != ev)
    {
        ...
        case NRF_DRV_TWIS_SUBSTATE_READ_PENDING:
               ...
               nrf_drv_twis_check_bit(ev, NRF_TWIS_EVENT_STOPPED))
            {
                ...
                NRFX_LOG_HEXDUMP_DEBUG((uint8_t *)p_reg->TXD.PTR, evdata.data.tx_amount * sizeof(p_reg->TXD.PTR));
                nrf_drv_call_event_handler(instNr, &evdata);
        ...
        nrf_drv_twis_event_handler_t    const event_handler)
{
    ...
    if ( m_var_inst[instNr].state != NRFX_DRV_STATE_UNINITIALIZED)
    {
        ...
        NRFX_LOG_WARNING("Function: %s, error code: %s.", (uint32_t)__func__, (uint32_t)NRFX_LOG_ERROR_STRING_GET(err_code));
        return err_code;
    }
    ...
    NRFX_LOG_INFO("Function: %s, error code: %s.", (uint32_t)__func__, (uint32_t)NRFX_LOG_ERROR_STRING_GET(err_code));
    return err_code;
}
